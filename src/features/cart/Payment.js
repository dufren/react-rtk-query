...
const Payment = () => {
  ...
  const [pay, { isLoading, isSuccess }] = usePaymentMutation();
  ...
  const isValid = (validValue, value) => {
    return !validValue && value.length > 0
      ? "border-pink-500 focus:ring-pink-500"
      : "focus:border-gray-600 focus:outline-none";
  };

  const validateName = isValid(validName, name);
  const validateCardNumber = isValid(validCardNumber, cardNumber);
  const validateExpDate = isValid(validExpirationDate, expirationDate);
  const validateCvv = isValid(validCvv, cvv);

  let canSave =
    [validName, validCardNumber, validExpirationDate, validCvv].every(
      Boolean
    ) &&
    !isLoading &&
    cart.length > 0;
    ...
    cart.length > 0 ? (
      cart.map((item) => {
        return (
          <div
            key={item.id}
            className="flex justify-between mb-10 shadow-xl p-2"
          >
      ...
      <div>
        <form onSubmit={formSubmitHandle}>
          <div className="p-10 rounded-md shadow-md bg-white">
            <h4 className="text-3xl text-gray-700 mb-10">
              Payment information
            </h4>
            <div className="mb-6">
              ...
              <Cleave
                className={`border outline-none rounded-md inline-block py-2 px-3 w-full text-gray-600 tracking-wider bg-gray-100 ${validateName}`}
                name="cardHolderName"
                value={name}
            ...
            <div className="mb-6">
              <div>
                ...
                <Cleave
                  className={`border outline-none rounded-md inline-block py-2 px-3 w-full text-gray-600 tracking-widest bg-gray-100 ${validateCardNumber}`}
                  name="cardNumber"
                  value={cardNumber}
